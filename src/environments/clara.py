...
class TransverseTuning(gym.Env):
    """
    Transverse beam parameter tuning environment for the CLARA S02 lattice

    Magnets: cla_s04_mag_quad_01, cla_s04_mag_quad_02, cla_s04_mag_quad_03
    Correctors: cla_s04_mag_hvcor_01
    Screen: cla_s04_dia_scr_03
    ...
    :param max_quad_setting: Maximum allowed quadrupole setting. The real quadrupoles
        can be set from -30 to 30. These limits are imposed by the power supplies, but
    ...
    :param max_steerer_setting: Maximum allowed steerer setting. The real steerers can
        be set from -5.30e-3 to 5.30e-3 rad. These limits are imposed by the power
        supplies.
    ...
    def __init__(
        ...
        max_quad_setting: float = 30.0,
        max_quad_delta: Optional[float] = None,
        max_steerer_setting: float = 5.30e-3,
    ...
    def _get_info(self):
        return {
            ...
            "magnet_names": [
                "cla_s04_mag_hvcor_01_h",
                "cla_s04_mag_hvcor_01_v",
                "cla_s04_mag_quad_01",
                "cla_s04_mag_quad_02",
                "cla_s04_mag_quad_03",
            ],
            ...
            "max_quad_setting": self.observation_space["magnets"].high[3],
...
class CheetahBackend(TransverseTuningBaseBackend):
    """
    Cheetah simulation backend to CLARA S02
    ...
    def __init__(
        ...
        self.segment = cheetah.Segment.from_lattice_json(
            Path(__file__).parent / "clara.json"
        )
        self.segment.cla_s04_dia_scr_03.is_active = True
    ...
    def is_beam_on_screen(self) -> bool:
        screen = self.segment.cla_s04_dia_scr_03
    ...
    def get_magnets(self) -> np.ndarray:
        return np.array(
            [
                self.segment.cla_s04_mag_hvcor_01.horizontal_angle,
                self.segment.cla_s04_mag_hvcor_01.vertical_angle,
                self.segment.cla_s04_mag_quad_01.k1,
                self.segment.cla_s04_mag_quad_02.k1,
                self.segment.cla_s04_mag_quad_03.k1,
            ]
        )

    def set_magnets(self, values: Union[np.ndarray, list]) -> None:
        self.segment.cla_s04_mag_hvcor_01.horizontal_angle = torch.tensor(
            values[0], dtype=torch.float32
        )
        self.segment.cla_s04_mag_hvcor_01.vertical_angle = torch.tensor(
            values[1], dtype=torch.float32
        )
        self.segment.cla_s04_mag_quad_01.k1 = torch.tensor(
            values[2], dtype=torch.float32
        )
        self.segment.cla_s04_mag_quad_02.k1 = torch.tensor(
            values[3], dtype=torch.float32
        )
        self.segment.cla_s04_mag_quad_03.k1 = torch.tensor(
            values[4], dtype=torch.float32
        )
    ...
    def reset(self, options=None) -> None:
        ...
        self.segment.cla_s04_mag_quad_01.misalignment = torch.tensor(
            misalignments[0:2], dtype=torch.float32
        )
        self.segment.cla_s04_mag_quad_02.misalignment = torch.tensor(
            misalignments[2:4], dtype=torch.float32
        )
        self.segment.cla_s04_mag_quad_03.misalignment = torch.tensor(
    ...
    def get_beam_parameters(self) -> np.ndarray:
        if self.simulate_finite_screen and not self.is_beam_on_screen():
            ...
            read_beam = self.segment.cla_s04_dia_scr_03.get_read_beam()
            return np.array(
    ...
    def get_misalignments(self) -> np.ndarray:
        return np.array(
            [
                self.segment.cla_s04_mag_quad_01.misalignment[0],
                self.segment.cla_s04_mag_quad_01.misalignment[1],
                self.segment.cla_s04_mag_quad_02.misalignment[0],
                self.segment.cla_s04_mag_quad_02.misalignment[1],
                self.segment.cla_s04_mag_quad_03.misalignment[0],
                self.segment.cla_s04_mag_quad_03.misalignment[1],
            ],
            dtype=np.float32,
        )
    ...
    def get_screen_image(self) -> np.ndarray:
        ...
        # scaling to 12 bits)
        return (
            (self.segment.cla_s04_dia_scr_03.reading).numpy() / 1e6 * 2**12
        )  # TODO Check scaling again.

    def get_binning(self) -> np.ndarray:
        return np.array(self.segment.cla_s04_dia_scr_03.binning)

    def get_screen_resolution(self) -> np.ndarray:
        return np.array(self.segment.cla_s04_dia_scr_03.resolution) / self.get_binning()

    def get_pixel_size(self) -> np.ndarray:
        return np.array(self.segment.cla_s04_dia_scr_03.pixel_size) * self.get_binning()

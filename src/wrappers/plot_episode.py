...
class PlotEpisode(gym.Wrapper):
    ...
    def __init__(
        ...
        self.show_rewards_only = show_rewards_only
        self.show_domain_randomisation_and_rewards = (
            show_domain_randomisation_and_rewards
        )
    ...
    def generate_episode_plot(
        ...
        fig = episode.plot_summary(
            save_path=save_path,
            show_rewards_only=show_rewards_only,
            show_domain_randomisation_and_rewards=show_domain_randomisation_and_rewards,
        )
        return fig
    ...
    def _save_episode_plot(self):
        ...
        fig = self.generate_episode_plot(
            save_path=file_path,
            show_rewards_only=self.show_rewards_only,
            show_domain_randomisation_and_rewards=self.show_domain_randomisation_and_rewards,
        )

        if self.log_to_wandb and wandb.run:
            wandb.log({"plots": fig}, commit=False)

        plt.close(fig)
        gc.collect()  # Manual gc for matplotlib cleanup

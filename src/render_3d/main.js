...
class SceneManager {
    constructor(containerId, language = 'en') {
        ...
        this.particleSpeed = 0.5;      // Units per frame (default: 0.1)
        this.scaleBeamPosition = 1;
        this.scaleBeamSpread = 1;
        ...
        // Model Management
        this.claraModel = null;
        this.undulatorModel = null;
        this.objToRender = "clara"; 
    ...
    createControlPanel() {
        ...
        const controls = [
            { id: 'HVCOR_01_H', type: 'angle', label: 'HVCOR_01_H', min: -5.30e-3, max: 5.30e-3, step: 1e-4, initial: 0 },
            { id: 'HVCOR_01_V', type: 'angle', label: 'HVCOR_01_V', min: -5.30e-3, max: 5.30e-3, step: 1e-4, initial: 0 },
            { id: 'QUAD_01', type: 'k1', label: 'QUAD_01', min: -30, max: 30, step: 0.01, initial: 0 },
            { id: 'QUAD_02', type: 'k1', label: 'QUAD_02', min: -30, max: 30, step: 0.01, initial: 0 },
            { id: 'QUAD_03', type: 'k1', label: 'QUAD_03', min: -30, max: 30, step: 0.01, initial: 0 },
            { id: 'particleSpeed', type: 'speed', label: 'Particle Speed', min: 0.001, max: 1.0, step: 0.001, initial: this.particleSpeed },
            { id: 'scaleBeamSpread', type: 'speed', label: 'Scale beam width', min: 1.0, max: 100.0, step: 1.0, initial: this.scaleBeamSpread },
            { id: 'scaleBeamPosition', type: 'speed', label: 'Scale beam position', min: 1.0, max: 101.0, step: 1, initial: this.scaleBeamPosition }
        ];
        ...
        controls.forEach(control => {
            ...
            input.addEventListener('input', () => {
                ...
                if (control.id === 'HVCOR_01_H' || control.id === 'HVCOR_01_V') {
    ...
    createPlotWindow() {
        ...
        const layout = {
            ...
            xaxis: {
                ...
                range: [-20, 20],  // Default range, will be updated dynamically
                showgrid: true,
                zeroline: false,
    ...
    loadModels() {
        ...
        for(let elementName in this.elementModels) {
            loader.load(
                `${baseUrl}assets/${elementName}.glb`, // No leading '/' here; baseUrl provides it
                (gltf) => {    
    ...
    toggleModel() {
        ...
        if (this.objToRender === "clara") {
            // Switch to Undulator
            this.objToRender = "undulator";
            this.claraModel.visible = false;
        ...
        if(!this.isSceneReady){
            for(let segmentIndex in data.segments){ 
                ...
                if(segment.segment_type in this.elementModels){
                    ...
                    meshcopy.position.set(0, 0, segment.element_position); 
    ...
    updateControls(changedControlId = null) {
        ...
        const controlValues = {
            HVCOR_01_H: parseFloat(this.controlSliders['HVCOR_01_H'].value),
            HVCOR_01_V: parseFloat(this.controlSliders['HVCOR_01_V'].value),
            QUAD_01: parseFloat(this.controlSliders['QUAD_01'].value),
            QUAD_02: parseFloat(this.controlSliders['QUAD_02'].value),
            QUAD_03: parseFloat(this.controlSliders['QUAD_03'].value),
        ...
        if (changedControlId) {
            ...
            if (changedControlId === 'stopSimulation') {
                ...
                return // don't send to the websocket if only particleSpeed changed, as it's local
            } else {
        ...
        if (this.ws && this.ws.readyState === WebSocket.OPEN) {
            console.log("sending control update to backend:", JSON.stringify(wsData));

...
def plot_screen_image(
    ...
    image_nan[image_filtered < 10] = (
        np.nan
    )  # TODO Undestand why this is lower than for ARES, (10).
    ...
    ax.imshow(
        ...
        cmap="rainbow",
        origin="lower",  # TODO Check if this is correct or if diag screen is wrong!
    )
    if title is None:
        title = ""
    ax.set_title(title + f" (Max Int.: {image_filtered.max():.3f})")
    ...
    if grid:
        ax.grid(visible=True, which="both", color="lightgrey", ls="--", lw=0.5)
...
def plot_beam_parameters_on_screen(
    ...
    ax.axvline(mu_x * 1e3, color=color, ls="--", alpha=1.0, label=label)
    ax.axhline(mu_y * 1e3, color=color, ls="--", alpha=1.0)

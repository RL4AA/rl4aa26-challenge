...
from ..environments.clara import TransverseTuning
...
class Episode:
    ...
    def plot_magnets(
        ...
        if normalize:
            ax.plot(magnets[:, 0] / 5.30e-3, c="#33bbee", label="CH")
            ax.plot(magnets[:, 1] / 5.30e-3, c=palette_colors[6], label="CV")
            ax.plot(magnets[:, 2] / 30.0, c=palette_colors[4], label="Q1")
            ax.plot(magnets[:, 3] / 30.0, c=palette_colors[5], label="Q2")
            ax.plot(magnets[:, 4] / 30.0, c="#ee3377", label="Q3")
            ...
            ax.plot(magnets[:, 2], c=palette_colors[4], label="Q1")
            ax.plot(magnets[:, 3], c=palette_colors[5], label="Q2")
            ax.plot([], c=palette_colors[6], label="CV")  # Dummy for legend
            ax.plot(magnets[:, 4], c="#ee3377", label="Q3")
            ax.plot([], c="#33bbee", label="CH")  # Dummy for legend
            ax.set_xlim(0, None)
            ax.set_ylim(-30 * 1.1, 30 * 1.1)
            if legend:
                ax.legend()

            ax_twinx = ax.twinx()

            ax_twinx.plot(magnets[:, 1] * 1e3, c=palette_colors[6], label="CV")
            ax_twinx.plot(magnets[:, 0] * 1e3, c="#33bbee", label="CH")
    ...
    def plot_misalignments(
        ...
        quadrupole_names = [
            name for name in self.infos[0]["magnet_names"] if "quad" in name
        ]
    ...
    def plot_quadrupoles(
        ...
        names = [name[-7:] for name in self.infos[0]["magnet_names"] if "quad" in name]
    ...
    def plot_steerers(
        ...
        names = [
            name[-10:] for name in self.infos[0]["magnet_names"] if "hvcor" in name
        ]
        ...
        if normalize:
            for name, history, color in zip(names, all_histories.T, palette_colors):
                ax.plot(history / 5.30e-3, c=color, label=name, ls="-")
            ax.set_xlim(0, None)
            ax.set_ylim(-1.1, 1.1)
            if legend:
                ax.legend(loc="upper right", ncol=len(names))
    ...
    def plot_summary(
        self,
        title: Optional[str] = None,
        figsize: tuple[float, float] = (510 / 72.72 * 1.9, 510 / 72.72 * 1.205 * 0.55),
        show_rewards_only: bool = False,
        ...
        if show_domain_randomisation_and_rewards or show_rewards_only:
            figsize = (figsize[0], figsize[1] * 1.5)

        # Plot Layout
        fig = plt.figure(figsize=figsize)
        grid_heigth = (
            4 if (show_domain_randomisation_and_rewards or show_rewards_only) else 2
        )

        # Small gap row between 2nd and 3rd content rows when domain randomisation and rewards are shown
        height_ratios = (
            [1, 1, 0.15, 1]
            if (show_domain_randomisation_and_rewards or show_rewards_only)
            else [1, 1]
        ...
        gs = fig.add_gridspec(
            ...
            hspace=0.1,
            wspace=0.15,  # Add horizontal spacing
        )
        ...
        if "screen_image" in self.infos[0]["backend_info"]:
            ...
            plot_beam_parameters_on_screen(
                mu_x=self.beam_parameters_before()[0],
                sigma_x=self.beam_parameters_before()[1],
                mu_y=self.beam_parameters_before()[2],
                sigma_y=self.beam_parameters_before()[3],
                resolution=self.infos[0]["screen_resolution"],
                pixel_size=self.infos[0]["pixel_size"],
                ax=ax_before,
                xlabel=False,
            ...
            plot_beam_parameters_on_screen(
                mu_x=self.beam_parameters_after()[0],
                sigma_x=self.beam_parameters_after()[1],
                mu_y=self.beam_parameters_after()[2],
                sigma_y=self.beam_parameters_after()[3],
                resolution=self.infos[0]["screen_resolution"],
                pixel_size=self.infos[0]["pixel_size"],
                ax=ax_after,
                xlabel=False,
        ...
        if show_domain_randomisation_and_rewards:
            ax_domain_randomisation = fig.add_subplot(gs[3, 0], sharex=ax_steerer)
            ax_rewards = fig.add_subplot(gs[3, 1], sharex=ax_steerer)
        ...
        elif show_rewards_only:
            ax_rewards_only = fig.add_subplot(gs[3, 1], sharex=ax_steerer)
            self.plot_rewards(
                ax=ax_rewards_only,
                xlabel=True,
                ylabel=True,
    ...
    def quadrupole_history(self) -> np.ndarray:
        ...
        magnets = self.magnet_history()
        quadrupole_indices = [
            i for i, name in enumerate(self.infos[0]["magnet_names"]) if "quad" in name
        ]
    ...
    def screen_image_shared_vmax(self) -> float:
        ...
        vmax = np.max(np.stack(processed_images))
        return vmax
    ...
    def steerer_history(self) -> np.ndarray:
        ...
        steerer_indices = [i for i, name in enumerate(magnet_names) if "hvcor" in name]
    ...
    def plot_objective_in_space(
        ...
        steerer_samples = np.linspace(-5.30e-3, 5.30e-3, num_objective_samples)
        ...
        axs[2, 0].imshow(
            ...
            aspect="auto",
            origin="lower",
            extent=[-30 / 30, 30 / 30, -30 / 30, 30 / 30],
            cmap=cmap,
        )
        ...
        axs[3, 0].scatter(
            [obs["magnets"][0] / 30 for obs in episode.observations],
            [obs["magnets"][4] / 5.30e-3 for obs in episode.observations],
            color="black",
            s=0.8,
        )
        axs[3, 0].scatter(
            [best_settings[0] / 30],
            [best_settings[4] / 5.30e-3],
            c="red",
            marker="*",
            s=20,
        )
        ...
        axs[3, 1].scatter(
            [obs["magnets"][1] / 30 for obs in episode.observations],
            [obs["magnets"][4] / 5.30e-3 for obs in episode.observations],
            color="black",
            s=0.8,
        )
        ...
        axs[3, 2].scatter(
            [obs["magnets"][2] / 5.30e-3 for obs in episode.observations],
            [obs["magnets"][4] / 5.30e-3 for obs in episode.observations],
            color="black",
            s=0.8,
        )
        axs[3, 2].scatter(
            [best_settings[2] / 5.30e-3],
            [best_settings[4] / 5.30e-3],
            c="red",
            marker="*",
            s=20,
        ...
        axs[3, 3].scatter(
            [obs["magnets"][3] / 30 for obs in episode.observations],
            [obs["magnets"][4] / 5.30e-3 for obs in episode.observations],
            color="black",
            s=0.8,
        )
        ...
        axs[2, 0].scatter(
            [best_settings[0] / 30], [best_settings[3] / 30], c="red", marker="*", s=20
        )
        ...
        axs[2, 1].scatter(
            [best_settings[1] / 30], [best_settings[3] / 30], c="red", marker="*", s=20
        )
        ...
        axs[0, 0].scatter(
            [best_settings[0] / 30], [best_settings[1] / 30], c="red", marker="*", s=20
        )
        axs[0, 0].set_ylabel(r"$Q_2$")

        fig.subplots_adjust(wspace=0.1, hspace=0.1)
        axs[3, 0].set_xlim(-30 / 30, 30 / 30)
        axs[3, 1].set_xlim(-30 / 30, 30 / 30)
        axs[3, 2].set_xlim(-1, 1)
        axs[3, 3].set_xlim(-30 / 30, 30 / 30)
        axs[2, 0].set_ylim(-30 / 30, 30 / 30)
        axs[1, 0].set_ylim(-1, 1)
        axs[0, 0].set_ylim(-30 / 30, 30 / 30)
    ...
    def sum_of_normalized_magnet_changes(self) -> float:
        ...
        """
        magnets = self.magnet_history()
        normalized_magnets = magnets / np.array([5.30e-3, 5.30e-3, 30, 30, 30])

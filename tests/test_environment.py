...
from src.environments import bc, clara, dl, sh
...
@pytest.mark.parametrize(
    "section",
    [
        pytest.param(clara, marks=pytest.mark.ea),
...
def test_check_env_ocelot():
    ...
    """
    env = clara.TransverseTuning(backend="ocelot")
    env = RescaleAction(env, -1, 1)  # Prevents SB3 action space scale warning
    check_env(env)
...
def test_passing_backend_args(section):
    ...
    if section in [clara, bc]:  # EA and BC with 3 quadrupoles + screen
...
@pytest.mark.parametrize(
    "section, settings",
    [
        pytest.param(clara, [1.0, 2.0, 1e-3, 3.0, 2e-3], marks=pytest.mark.ea),
